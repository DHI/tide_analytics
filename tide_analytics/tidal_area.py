...
class TidalArea:
    ...
    def tidal_analysis(self):
        ...
        for element in tqdm(element_range):
            ...
            try:
                ...
                out = self.tidal_analysis_series(element_se, element_cs, element_cd, throw_error)
                tidal_characteristics_list.append({'element': element, **out.tidal_characteristics.to_dict()})
                tidal_errors_list.append({'element': element, **out.tidal_errors.to_dict()})
            except Exception as e:
                if throw_error:
                    self.plot_element_on_map(element)
                    print(f"{type(e).__name__}")
                    print(e)
                    self.error_support_prompt(pd.DataFrame(tidal_errors_list).set_index('element').sum(axis=0), len(tidal_errors_list))
                    raise e
                skipped_errors += 1
    ...
    def _parse_input(self, surface_elevation: mikeio.DataArray, 
        ...
        surface_elevation = Variable(data=surface_elevation, unit = surface_elevation.unit.short_name)
        ...
        if (current_speed is not None and current_direction is not None):
            if current_direction.unit == EUMUnit.radian:
                ...
                current_direction.item = ItemInfo("Current direction", current_direction.item.type, EUMUnit.degree)
            current_speed = Variable(data = current_speed, unit = current_speed.unit.short_name)
            current_direction = Variable(data = current_direction, unit = current_direction.unit.short_name)
            ...
            raise ValueError("Either current_speed and current_direction, or velocity_x and velocity_y must be provided.")
    ...
    def _convert_uv_to_speeddirection(self, velocity_x, velocity_y) -> tuple[mikeio.DataArray, mikeio.DataArray]:
        ...
        ds_speed = mikeio.DataArray(speed, time= velocity_x.time, item = ItemInfo("Current speed", EUMType.Current_Direction, EUMUnit.meter_per_sec), geometry=velocity_x.geometry)
    ...
    def plot_element_on_map(self, element, figsize:tuple[float, float]=(12,12), show = True):
    ...
    def plot_errormap(self, figsize:tuple[float, float]=(12,12)):
        ...
        if self.tidal_errors is None:
            raise ValueError("Tidal errors have not been calculated yet. Please run the tidal analysis first.")

        error_to_color = {field.name: idx for idx, field in enumerate(fields(TidalErrors))}
        ...
        for error, color_idx in error_to_color.items():
            if error in self.tidal_errors.columns:
                ...
                raise ValueError(f"Error {error} not found in tidal_errors. Please add it.")
        ...
        dataarray = (DataArray(data, time=str(self.surface_elevation.data.time.mean()), 
        ...
        ax.legend(handles=handles, labels=present_errors, loc='upper center', 
                bbox_to_anchor=(0.5, -0.15), ncol=3)

        if self.save_path_parent_folder is not None:
            self.save_path_parent_folder.mkdir(parents=True, exist_ok=True)
            save_path = self.save_path_parent_folder / f"tidal_errors_{self.identifier}.png"
    ...
    def plot_statistical_summary(self, figsize: tuple[float, float] = (16,6)):
        ...
        df_timedelta_seconds = df[timedelta_cols].apply(lambda col: col.dt.total_seconds() / 3600) 

        scaler = MinMaxScaler()
        scaled_data = scaler.fit_transform(pd.concat([df[float_cols], df_timedelta_seconds], axis=1))
        scaled_df = pd.DataFrame(scaled_data, columns=float_cols + timedelta_cols)

        assert all(df.columns == scaled_df.columns), "The columns of the dataframe and the scaled dataframe are not the same"

        fig , ax = plt.subplots(figsize= figsize)
        _ , boxplot = scaled_df.boxplot(grid=False, return_type='both', notch = True, patch_artist=True,
                                        boxprops = {"edgecolor": "black"},
                                        flierprops={'marker': '.', 'markersize': 7, 'markerfacecolor': 'grey', 'markeredgecolor': 'None', 'alpha': 0.5})
        ...
        if self.save_path_parent_folder is not None:
            if self.save_path_parent_folder is not None:
                self.save_path_parent_folder.mkdir(parents=True, exist_ok=True)
            save_path = self.save_path_parent_folder / f"statistical_summary_{self.identifier}.png"
            fig.savefig(save_path, dpi=600)
